...
function SidebarItem({ text, menuIcon, active, link, space = 'space-x-1', dropdown = false }) {
    ...
    if (dropdown) {
        return (
            <div className="mb-4" onClick={() => setIsDropdownOpen(!isDropdownOpen)}>
                <div className={`flex  items-center justify-between`}>
                    ...
                    <svg className={`w-4 h-4 ml-2 fill-secondary ${isDropdownOpen ? 'rotate-180' : ''} transition-transform duration-300`} viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
        ...
        return (
            <div className={`flex ${space}`}>{menuIcon}
                <li className="mb-[10px]"><Link to={link} className={`text-sm ${active ? 'text-main' : 'text-secondary'} `}>{text}</Link></li>
            </div>
        )
    }
}
