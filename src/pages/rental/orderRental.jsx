...
export default function OrderRental() {
  ...
  const { quill: quillAlamat, quillRef: quillRefAlamat } = useQuill();
  const {
    quill: quillAlamatKeberangkatan,
    quillRef: quillRefAlamatKeberangkatan,
  } = useQuill();
  ...
  useEffect(() => {
    if (quillAlamat) {
      quillAlamat.on("text-change", () => {
        setForm((prevData) => ({
          ...prevData,
          alamat: quillAlamat.root.innerHTML,
        }));
      });
    }

    if (quillAlamatKeberangkatan) {
      quillAlamatKeberangkatan.on("text-change", () => {
        setForm((prevData) => ({
          ...prevData,
          alamat_keberangkatan: quillAlamatKeberangkatan.root.innerHTML,
        }));
      });
    }
  }, [quillAlamat, quillAlamatKeberangkatan]);
